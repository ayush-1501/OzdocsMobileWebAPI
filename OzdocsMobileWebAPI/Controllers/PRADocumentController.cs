...
namespace OzdocsMobileWebAPI.Controllers
{
    ...
    public class PRADocumentController : ControllerBase
    {
        ...
        [HttpGet("GetPRADataRecordCount")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(PRADocument))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(Response))]
        public IActionResult GetPRADataRecordCount(DateTime? toDate, DateTime? fromDate, string? TypeOfQuery, string? Id)
        {
            Response response = new Response();
            DataTable retData;
            string json;
            ...
            {
                DataAccess dataAccess = new DataAccess(_configuration);
                retData = dataAccess.GetPRADocumentRecordCount(toDate, fromDate, TypeOfQuery, Id);
